...
#pragma warning disable 1998
    ...
    public class HockeyPlatformHelperWPF : IHockeyPlatformHelper
    {
        ...
        private string PostfixWithAppId(string folderName, bool noDirectorySeparator = false)
        {
        ...
        {
            using (var fileStream = isoStore.OpenFile(PostfixWithAppId(FILE_PREFIX + key, true), FileMode.Create, FileAccess.Write))
            {
        ...
        public string GetSettingValue(string key)
        {
            if(isoStore.FileExists(FILE_PREFIX + key)) {
                using (var fileStream = isoStore.OpenFile(PostfixWithAppId(FILE_PREFIX + key, true), FileMode.Open, FileAccess.Read))
                {
        ...
        {
            if (isoStore.FileExists(PostfixWithAppId(FILE_PREFIX + key, true)))
            {
                isoStore.DeleteFile(PostfixWithAppId(FILE_PREFIX + key, true));
            }
        }
        ...
        {
            if (isoStore.FileExists(PostfixWithAppId(folderName) + Path.DirectorySeparatorChar + fileName))
            {
                isoStore.DeleteFile(PostfixWithAppId(folderName) + Path.DirectorySeparatorChar + fileName);
                return true;
            }
            return false;
        }
        ...
        {
            return isoStore.FileExists(PostfixWithAppId(folderName) + Path.DirectorySeparatorChar + fileName);
        }
        ...
        {
            return isoStore.OpenFile(PostfixWithAppId(folderName) + Path.DirectorySeparatorChar + fileName, FileMode.Open, FileAccess.Read);
        }
        ...
        public async Task WriteStreamToFileAsync(Stream dataStream, string fileName, string folderName = null)
        {
            ...
            if (!isoStore.DirectoryExists(PostfixWithAppId(folderName))) {
                isoStore.CreateDirectory(PostfixWithAppId(folderName));
            }

            using (var fileStream = isoStore.OpenFile(PostfixWithAppId(folderName) + Path.DirectorySeparatorChar + fileName,FileMode.Create,FileAccess.Write)) {
        ...
        {
            try {
                return isoStore.GetFileNames(PostfixWithAppId(folderName) + Path.DirectorySeparatorChar + fileNamePattern ?? "*");
